...
> 评估日期：2026-08-30。当前分数只计入已完成的本地实测；公网安装、GitHub Actions 和模型对照尚未计分。
...
| 实测表现 | 23 | 19 | 双 Python 版本、本地浏览器、四类结构校验 | 公网安装、线上 CI、Claude 模型对照未完成 |
| 反例与黑名单 | 7 | 7 | SKILL、README、安全文档和测试均覆盖 | 无 |
| **总分** | **100** | **93** | **本地实测** | **发布后项目尚未计分** |

## 5. 差距清单

### P0：公开前必须完成

- 确认私有 GitHub 仓库的线上 CI 真实通过。
...
| `scripts/research_gate.py` | 仓库 CLI 入口 | 20 项测试与端到端回放 |
